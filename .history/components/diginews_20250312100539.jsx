      ...
      {authToken ? (
        <div className="relative flex-1 sm:flex-auto" ref={userMenuRef}>
          <motion.button
            whileHover={{ scale: 1.03 }}
            onClick={() => setIsUserMenuOpen(prev => !prev)}
            className="w-full sm:w-auto flex items-center gap-2 px-4 py-2.5 bg-green-50 border border-green-200 rounded-full text-green-600 font-medium hover:bg-green-100 transition-all duration-200 shadow-sm"
          >
            <div className="h-8 w-8 rounded-full bg-green-100 flex items-center justify-center text-green-600 font-medium">
              {userEmail?.[0]?.toUpperCase() || 'U'}
            </div>
