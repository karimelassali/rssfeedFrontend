...
import { ArrowLeft, Cog, LogOut } from "lucide-react";
    ...
    if (activeFilters.length > 0) {
      results = results.filter(item =>
        ...
        activeFilters.some(filter =>
          typeof filter === 'object' && filter.type === 'source' && filter.value
            ? item.source === filter.value
            : item.source === filter
        )
      );
    }
  ...
  const fetchData = async (page = 1, isLoadMore = false) => {
    ...
    try {
      ...
      if (isLoadMore) {
        setAllData(prevData => [
          ...prevData,
          ...filteredNewData.filter(item => !prevData.some(prev => prev.id === item.id))
        ]);
      } else {
        ...
        {currentPage < lastPage && (
          ...
          >
            {isLoadingMore ? (
              ...
              <>
                <span>Carica altri articoli ({filteredData.length})</span>
