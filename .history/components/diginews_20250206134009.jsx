...
/*************  ✨ Codeium Command 🌟  *************/
    (<div className="max-w-2xl mx-auto bg-white min-h-screen flex flex-col">
      ...
      <div className="flex-1 border-red-400 p-4 md:p-6 pt-0">
        <div className="space-y-6">
          {newsItems.map((item) => (
            <article
              key={item.id}
              className="border-b w-8 border-red-200 pb-6 last:border-0  break-words relative">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  {/* Source */}
                  <p className="text-sm font-medium mb-1 overflow-hidden text-ellipsis whitespace-nowrap">{item.source}</p>
                  <p className="text-sm font-medium mb-1">{item.source}</p>

                  {/* Title */}
                  <h2 className="text-base font-semibold mb-2 break-words overflow-hidden text-ellipsis whitespace-normal">{item.title}</h2>
                  <h2 className="text-base font-semibold mb-2 break-words">{item.title}</h2>

                  {/* Time */}
                  <p className="text-sm text-gray-500 overflow-hidden text-ellipsis whitespace-nowrap">{item.time}</p>
...
/******  f97acb2f-cea5-49d2-949c-e0ba5f9c9cd9  *******/
  );
}
