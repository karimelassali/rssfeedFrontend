...
export async function POST(request, { params }) {
    const { id } = params;
    const {title , description , image,category,showInHomePage, publishType,date,time,scheduledTime} = await request.json();
    console.log(title, description,image,category,showInHomePage, publishType,date,time,scheduledTime);
    try {
